...
|------|--------|------------|-----------|
| **Authenticator protocol** | `authenticate(headers: dict[str, str]) -> Awaitable[Identity \| None]` (async, headers-map) | `authenticate(headers: Record<string, string>): Promise<Identity \| null>` (async, headers-map) | Post-JWT-1 unification (CHANGELOG 0.14.0): all three SDKs accept a pre-extracted headers map and return an awaitable. Rust uses `authenticate(headers: &HashMap<String, String>) -> impl Future<Output = Option<Identity>>`. This is no longer an intentional difference — the contract is unified across SDKs. |
