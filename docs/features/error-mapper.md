...
- `userFixable` — **all three now read it off the error object rather than hardcoding it**, so a code that carries `user_fixable` gets the key and one that does not, does not. The attribute name differs by upstream: Python reads snake_case `error.user_fixable` and maps it to camelCase (`errors.py:135`); TypeScript reads the already-camelCase `error.userFixable` (`errors.ts:397`, driven by `_AI_GUIDANCE_FIELDS`); Rust copies `error.user_fixable` when the response has not already set one (`errors.rs:314`). An earlier revision of this spec said `userFixable: true` was "TypeScript only, hardcoded at errors.ts:241,273" — that has not been true since the `_AI_GUIDANCE_FIELDS` refactor. The practical consequence is unchanged: a code whose upstream error class does not set `user_fixable` (e.g. `DependencyNotFoundError` in apcore-python) yields a response with no `userFixable` key at all
